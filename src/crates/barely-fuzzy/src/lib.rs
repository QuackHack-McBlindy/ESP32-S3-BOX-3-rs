...
/// RETURNS UP TO `n` BEST MATCHING CANDIDATES, SORTED BY SIMILARITY (HIGHEST FIRST)
/// USES THE SAME TWO-STAGE PIPELINE AS `best_fuz`:
/// 1. normalise input and each candidate (lowercase).
/// 2. quick trigram filter – skip if trigram similarity < `trigram_threshold`.
/// 3. for survivors, compute Levenshtein similarity.
/// 4. keep the top `n` results.
///
/// IF `n` IS 0, RETURNS AN EMPTY Vec.  
/// CANDIDATES THAT PASS THE TRIAGRAM FILTER BUT HAVE EQUAL SCORES ARE KEPT
/// IN THE ORDER THEY WERE ENCOUNTERED (STABLE ORDERING AMONG EQUAL SCORES IS NOT GUARANTEED).
#[cfg(feature = "alloc")]
pub fn best_fuz_n<'a>(
    input: &[u8],
    candidates: &'a [&[u8]],
    ...
    let norm_input = normalize_ascii_lowercase(input);
    let mut results: Vec<(&[u8], u8)> = Vec::with_capacity(n);

    for &candidate in candidates {
        let norm_cand = normalize_ascii_lowercase(candidate);
        let tri = trigram_similarity(&norm_input, &norm_cand);
        if tri < trigram_threshold {
            continue;
        }
        let score = levenshtein_similarity(&norm_input, &norm_cand);

        // INSERT CANDIDATES INTO RESULTS, KEEPING THE VEC SORTED (DESCENDING BY SCORE)
        // AND LIMITED TO `n` ELEMENTS. FOR SMALL N THIS IS CHEAP.
        let pos = results.partition_point(|&(_, s)| s > score);
        if results.len() < n {
            results.insert(pos, (candidate, score));
        } else if pos < n {
            // SHIFT OUT THE LOWEST SCORE (LAST ELEMENT) & INSERT AT CORRECT POSITION
            results.pop();
            results.insert(pos, (candidate, score));
        }
    }
