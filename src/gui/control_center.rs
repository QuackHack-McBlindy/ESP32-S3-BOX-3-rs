...
// ───────────────────────────────────────────────────────────────────────
// BUTTON 1: (AIRPLANE)  LOW POWER MODE - CLOCKS DOWN TO 180 MHZ & DISABLES EVERYTHING
// BUTTON 2: (LIGHTBULB) TOGGLES ALL APARTMENT LIGHTS ON/OFF 
// BUTTON 3: (PHONE)     LOCATES PHONE
// BUTTON 4: (GEAR)      OPENS SETTINGS
// ───────────────────────────────────────────────────────────────────────
...
static mut API_BUF: [u8; 512] = [0u8; 512];
...
// ───────────────────────────────────────────────────────────────────────
// HELPERS
pub async fn apply_lights_scene() {
    crate::applications::zigduck::HOME_CMD.send(crate::applications::zigduck::HomeCommand::ToggleLights).await;
}
...
fn render_panel_to_buffer(buf: &mut [u16], screen_w: usize, panel_h: usize) {
    ...
    // GLOW EFFECT FOR LIGHTBULB IF LIGHTS ARE ON    
    if lights_state { 
        use embedded_graphics::pixelcolor::Rgb565;
        use embedded_graphics::prelude::IntoStorage;
    
        let i = 1;
        let x0 = (box_margin + i * (box_w + box_spacing)).max(0);
        let x1 = (x0 + box_w).min(w);
    
        let dark   = Rgb565::new(128, 64, 0);
        let bright = crate::gui::colors::YELLOW;
        let glow_colors = [
            crate::gui::colors::blend(dark, bright, 40),
            crate::gui::colors::blend(dark, bright, 120),
            crate::gui::colors::blend(dark, bright, 200),
            bright,
        ];
        let glow_radius = glow_colors.len() as i32;
        let glow_u16: [u16; 4] = glow_colors.map(|c| c.into_storage());
    
        for step in 0..glow_radius {
            let offset = glow_radius - step - 1;
            let gx0 = x0 - offset;
            let gy0 = box_y - offset;
            let gx1 = x1 + offset;
            let gy1 = box_y + box_h + offset;
            let color = glow_u16[step as usize];   // ← cast to usize
    
            for y in gy0.max(0)..gy1.min(h) {
                let row_start = (y as usize) * screen_w;
                for x in gx0.max(0)..gx1.min(w) {
                    let idx = row_start + x as usize;
                    let on_perimeter = y == gy0 || y == gy1 - 1 || x == gx0 || x == gx1 - 1;
                    if on_perimeter {
                        buf[idx] = color;
                    }
                }
    ...
    let mut draw_icon = |png: &embedded_png::Png, box_index: i32, tint: Option<u16>| {
        ...
        if let Some(color) = tint {
            draw_scaled_png_raw_tinted(buf, png, icon_x, icon_y, icon_scale, screen_w, panel_h, color);
        } else {
            draw_scaled_png_raw(buf, png, icon_x, icon_y, icon_scale, screen_w, panel_h);
        }
    };

    // AIRPLANE BUTTON (RED WHEN LOW-POWER MODE IS ACTIVE)
    if let Some(power) = &power_icon {
        let tint = if power_state { Some(0xF800u16) } else { None };
        draw_icon(power, 0, tint);
    }

    // LIGHTS BUTTON (YELLOW WHEN LIGHTS IS ON)
    let lights_icon = if lights_state { llghts_on_icon.as_ref() } else { llghts_off_icon.as_ref() };
    if let Some(lights) = lights_icon {
        let yellow_tint = crate::gui::colors::YELLOW.into_storage();
        let tint = if lights_state { Some(yellow_tint) } else { None };
        draw_icon(lights, 1, tint);
    }

    // PHONE & SETTINGS NOT TINTED
    if let Some(phone) = &phone_icon {
        draw_icon(phone, 2, None);
    }
    if let Some(settings) = &settings_icon {
        draw_icon(settings, 3, None);
    }
