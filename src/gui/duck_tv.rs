// GUI/DUCK_TV
// DRAWS THE DUCK-TV APPLICATION IN THE GUI
// WRITES PNG PIXELS DIRECTLY INTO THE RAW FRAMEBUFFER
// PREV/NEXT TRACK+PLAY/PAUSE CONTROL BUTTONS
// TAP MUSIC/TV/MOVIE ICONS TO SPLIT THE PLAYER INTO TWO PIECES THAT SLIDE APART AND REVEAL THE MEDIA BROWSER
// TAP ON CHOSEN MEDIA WILL DIRECTLY CAST TO THE TV
...
static mut PNG_MUSIC: Option<(&'static [u16], u32, u32)> = None;
static mut PNG_MOVIES: Option<(&'static [u16], u32, u32)> = None;
static mut PNG_HEART_FILLED: Option<(&'static [u16], u32, u32)> = None;
static mut PNG_TV: Option<(&'static [u16], u32, u32)> = None;
static mut PNG_POWER: Option<(&'static [u16], u32, u32)> = None;
...
static LAST_TV_ON: core::sync::atomic::AtomicBool = core::sync::atomic::AtomicBool::new(false);
...
pub fn init() {
    ...
    unsafe {
        ...
        PNG_MUSIC        = Some(decode_and_leak(crate::base::assets::DUCKTV_MUSIC_PNG));
        PNG_MOVIES       = Some(decode_and_leak(crate::base::assets::DUCKTV_MOVIES_PNG));
        PNG_HEART_FILLED = Some(decode_and_leak(crate::base::assets::DUCKTV_MOVIES_PNG));
        PNG_TV           = Some(decode_and_leak(crate::base::assets::DUCKTV_TV_PNG));
        PNG_POWER        = Some(decode_and_leak(crate::base::assets::SETTINGS_SLEEP_PNG));
    }

    let screen_w = crate::state::LCD_WIDTH as usize;
    let screen_h = crate::state::LCD_HEIGHT as usize;

    let title_opt: Option<alloc::string::String> = None;
    let tv_is_on = crate::load!(crate::state::TV_IS_ON);
    let mut buf = alloc::vec![0u16; screen_w * screen_h];
    render_base_to_buffer(&mut buf, screen_w, screen_h, &title_opt, false, false, tv_is_on);
    ...
    LAST_TV_ON.store(tv_is_on, core::sync::atomic::Ordering::Release); 
...
static mut HIT_AREAS: core::option::Option<[crate::gui::HitArea; 7]> = core::option::Option::None;
...
pub fn set_hit_areas_closed() {
    const AREAS: [crate::gui::HitArea; 7] = [
        // PREVIOUS TRACK BUTTON
        crate::gui::HitArea {
            x: 20, y: 382,
            width: 90, height: 90,
            action: crate::gui::TouchAction::TvPrev,
        },
        // PLAY / PAUSE BUTTON
        crate::gui::HitArea {
            x: 160, y: 382,
            width: 90, height: 90,
            action: crate::gui::TouchAction::TvPlayPause,
        },
        // NEXT TRACK BUTTON
        crate::gui::HitArea {
            x: 300, y: 382,
            width: 90, height: 90,
            action: crate::gui::TouchAction::TvNext,
        },
        // BROWSE MUSIC
        crate::gui::HitArea {
            x: 5, y: 130,
            width: 90, height: 90,
            action: crate::gui::TouchAction::TvMusic,
        },
        // BROWSE MOVIES
        crate::gui::HitArea {
            x: 315, y: 130,
            width: 90, height: 90,
            action: crate::gui::TouchAction::TvMovies,
        },        
        // BROWSE TV-SHOWS
        crate::gui::HitArea {
            x: 115, y: 85,
            width: 180, height: 180,
            action: crate::gui::TouchAction::TvShows,
        },
        // POWER
        crate::gui::HitArea {
            //x: 294, y: 10,
            x: 345, y: 25,
            width: 45, height: 45,
            action: crate::gui::TouchAction::TvPower,
        },
    ];
...
pub fn close_split() {
    critical_section::with(|cs| {
        ...
        // CLEAR BROWSE STATE WHEN CLOSING
        crate::applications::duck_tv::BROWSE_CATEGORY.borrow_ref_mut(cs).clear();
        crate::applications::duck_tv::BROWSE_PATH.borrow_ref_mut(cs).clear();
        crate::applications::duck_tv::BROWSE_ITEMS.borrow_ref_mut(cs).clear();
    });
}
...
// ───────────────────────────────────────────────────────────
// PLAYLIST SCROLLING
pub struct MediaScroll {
    pub offset: i32,
    pub target: i32,
    pub max_scroll: i32,
}

pub(crate) static MEDIA_SCROLL: critical_section::Mutex<core::cell::RefCell<MediaScroll>> =
    critical_section::Mutex::new(core::cell::RefCell::new(MediaScroll {
        offset: 0,
        target: 0,
        max_scroll: 0,
    }));

pub fn animate_media_scroll(speed: i32) {
    critical_section::with(|cs| {
        let mut scroll = MEDIA_SCROLL.borrow_ref_mut(cs);
        let diff = scroll.target - scroll.offset;
        if diff != 0 {
            let step = diff.clamp(-speed, speed);
            scroll.offset += step;
            defmt::debug!("scroll offset={} target={} (diff={}, step={})", scroll.offset, scroll.target, diff, step);
        }
    });
}
...
fn render_base_to_buffer(
    ...
    let (clear_data, clear_w_u32, clear_h_u32) = unsafe { PNG_MUSIC.unwrap() };
    let (heart_data, heart_w_u32, heart_h_u32) = unsafe { PNG_MOVIES.unwrap() };
    let (album_data, album_w_u32, album_h_u32) = unsafe { PNG_TV.unwrap() };
    let (power_data, power_w_u32, power_h_u32) = unsafe { PNG_POWER.unwrap() };
    ...
    let scale_tv: f32 = 0.5;
    let dst_w = (album_w_u32 as f32 * scale_tv) as i32;
    let dst_h = (album_h_u32 as f32 * scale_tv) as i32;
    ...
    // TV  (SCALE 0.5)
    let x = w / 2 - dst_w / 2;
    let album_y = (h as f32 * 0.35) as i32 - dst_h / 2;
    draw_raw_pixels(buf, album_data, album_w_u32, album_h_u32, x, album_y, scale_tv, screen_w, screen_h);

    // POWER BUTTON
    let power_scale: f32 = 0.25;
    let power_dst_w = (power_w_u32 as f32 * power_scale) as i32;
    let power_x = w - power_dst_w - 25;
    let power_y = 25;

    let power_tint = if tv_is_on { 0x07E0 } else { 0xF800 };
    draw_raw_pixels_tinted(
        buf,
        power_data,
        power_w_u32,
        power_h_u32,
    ...
    // SIDE BUTTONS: MUSIC/MOVIES (SCALE 0.5)
    ...
    let clear_y = album_y + (dst_h - clear_h_scaled) / 2;
    let heart_x = x + dst_w + side_gap;
    let heart_y = album_y + (dst_h - heart_h_scaled) / 2;
...
pub fn draw(fb: &mut crate::components::framebuffer::Framebuffer) {
    ...
    let tv_is_on = crate::load!(crate::state::TV_IS_ON);
    let title_opt: Option<alloc::string::String> = None;
    let total_duration = 0u32;
    let current_position = 0u32;
    ...
    let tv_on_changed = LAST_TV_ON.load(core::sync::atomic::Ordering::Acquire) != tv_is_on;

    if title_changed || tv_on_changed || playing_changed || liked_changed {
    ...
    if BASE_DIRTY.swap(false, core::sync::atomic::Ordering::AcqRel) {
        critical_section::with(|cs| {
            ...
            let mut buf = alloc::vec![0u16; screen_w * screen_h];
            render_base_to_buffer(&mut buf, screen_w, screen_h, &title_opt, is_playing, is_liked, tv_is_on);
            *cache = Some(buf);
        });
    }

        
    // REBUILD PLAYLIST (OR BROWSE) CACHE IF DIRTY
    if PLAYLIST_DIRTY.swap(false, core::sync::atomic::Ordering::AcqRel) {
        critical_section::with(|cs| {
            let mut cache = PLAYLIST_CACHE.borrow_ref_mut(cs);
    
            let browse_items = crate::applications::duck_tv::BROWSE_ITEMS.borrow_ref(cs).clone();
            let category = crate::applications::duck_tv::BROWSE_CATEGORY.borrow_ref(cs).clone();
    
            let mut owned_titles: heapless::Vec<heapless::String<128>, 64> = heapless::Vec::new();
            if !category.is_empty() {
                for item in browse_items.iter() {
                    let _ = owned_titles.push(item.clone());
                }
            }
    
            // CREATE &str REF 
            let title_strs: heapless::Vec<&str, 64> = owned_titles.iter().map(|s| s.as_str()).collect();
    ...
    if offset == 0 {
        ...
        if gap_top < gap_bottom {
            ...
            let scroll_off = critical_section::with(|cs| {
                let s = MEDIA_SCROLL.borrow_ref(cs);
...
pub fn handle_media_tap(x: i32, y: i32) -> Option<usize> {
    let offset = critical_section::with(|cs| MEDIA_SPLIT.borrow_ref(cs).current_offset);
    if offset <= 0 {
        return None;
    }
    ...
    let split_line_y = crate::state::LCD_HEIGHT as i32 / 2;
    let gap_top = split_line_y - offset;
    if y < gap_top || y > split_line_y + offset {
        return None;
    }

    let scroll_off = critical_section::with(|cs| MEDIA_SCROLL.borrow_ref(cs).offset);
    ...
    let total = critical_section::with(|cs| {
        crate::applications::duck_tv::BROWSE_ITEMS.borrow_ref(cs).len()
    });
