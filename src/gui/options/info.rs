...
pub fn handle_swipe(
    ...
    match direction {
        ...
        crate::components::gt911::SwipeDirection::Left
        | crate::components::gt911::SwipeDirection::Right => {
            if is_on_info_page() {
                true
            } else {
                false
...
fn draw_info_page(
    ...
    match page_num {
        ...
        1 => {
            ...
            let ssid_line = critical_section::with(|cs| {
                let guard = crate::state::CONNECTED_SSID.borrow_ref(cs);
                if let Some(ssid) = guard.as_ref() {
