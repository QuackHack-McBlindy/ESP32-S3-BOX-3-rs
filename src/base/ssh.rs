...
const EMBEDDED_CLIENT_KEY: [u8; 32] = match crate::state::SSH_PRIVATE_KEY_HEX {
    Some(hex) => hex_to_bytes(hex),
    None => [0u8; 32],
};

static CLIENT_KEY: embassy_sync::once_lock::OnceLock<sunset::SignKey> = embassy_sync::once_lock::OnceLock::new();
...
fn simple_base64_encode(input: &[u8]) -> alloc::string::String {
    ...
    for &byte in input {
        ...
        while bits >= 6 {
            bits -= 6;
            out.push(CHARS[((buffer >> bits) & 0x3F) as usize] as char);
        }
    }
    if bits > 0 {
        buffer <<= 6 - bits;
        out.push(CHARS[(buffer & 0x3F) as usize] as char);
    }
    while out.len() % 4 != 0 {
        out.push('=');
    }
...
// ───────────────────────────────────────────────────────────────────────
// CLIENT
fn get_client_key() -> &'static sunset::SignKey {
    CLIENT_KEY.get_or_init(|| {
        if EMBEDDED_CLIENT_KEY != [0u8; 32] {
            let signing_key = ed25519_dalek::SigningKey::from_bytes(&EMBEDDED_CLIENT_KEY);
            sunset::SignKey::Ed25519(signing_key)
        } else {
            defmt::warn!("NO EMBEDDED CLIENT KEY – GENERATING A TEMPORARY ONE!");
            let key = sunset::SignKey::generate(sunset::KeyType::Ed25519, None)
                .expect("RNG FAILURE");
            if let sunset::SignKey::Ed25519(k) = &key {
                let public = k.verifying_key().to_bytes();
                let secret = k.to_bytes();
                defmt::info!(
                    "TEMPORARY CLIENT KEY PUB: {:?}, SECRET: {:?}",
                    public, secret
                );
                defmt::info!("COPY THE SECRET BYTES AND SET EMBEDDED_CLIENT_KEY TO MAKE IT PERMANENT.");
            }
            key
        }
    })
...
// ─────────────────────────────────────────────────────────────────
// SSH CLIENT TASK – CONNECTS TO A REMOTE SERVER
#[embassy_executor::task]
pub async fn ssh_client_task(
    stack: &'static embassy_net::Stack<'static>,
    server_addr: embassy_net::IpEndpoint,
) {
    // SOCKET BUFFERS
    ...
    if let Err(e) = socket.connect(server_addr).await {
        defmt::error!("SSH CLIENT CONNECT ERROR: {}", defmt::Debug2Format(&e));
        return;
    }
    defmt::info!("SSH CLIENT CONNECTED TO {}", server_addr);

    let (mut rx, mut tx) = socket.split();

    let client: &'static sunset_async::SSHClient<'static> = alloc::boxed::Box::leak(alloc::boxed::Box::new(sunset_async::SSHClient::new_owned()));

    let run_fut = client.run(&mut rx, &mut tx);

    let progress_fut = async {
        let mut ph = sunset_async::ProgressHolder::new();

        loop {
            match client.progress(&mut ph).await {
                Ok(event) => match event {
                    // ─────────────────────────────────────────────────────────────────
                    // STEP 1: CHECK HOST KEY
                    sunset::event::CliEvent::Hostkey(h) => {
                        let server_key = h.hostkey().unwrap();
                        if let sunset::PubKey::Ed25519(k) = &server_key {
                            let raw = k.key.0;
                            ...
                            let b64 = simple_base64_encode(&blob);
                            let key_line = b64.as_str();

                            let known1 = crate::state::SSH_KNOWN_HOST.unwrap_or("");
                            let known2 = crate::state::SSH_KNOWN_HOST2.unwrap_or("");
                            let known3 = crate::state::SSH_KNOWN_HOST3.unwrap_or("");

                            if key_line == known1 || key_line == known2 || key_line == known3 {
                                defmt::info!("HOST KEY MATCHES KNOWN HOST");
                                h.accept().unwrap();
                            } else {
                                // FOR SECURITY WE REJECT UNKNOWN HOSTS:
                                h.reject().unwrap();
                                return;
                                // OR ACCEPT WITH WARNING:
                                // defmt::warn!("UNKNOWN HOST KEY! ({}), ACCEPTING ANYWAY", key_line);
                                // h.accept().unwrap();
                            }
                        } else { h.accept().unwrap(); }
                    }
                    // ─────────────────────────────────────────────────────────────────
                    // STEP 2: SEND USERNAME
                    sunset::event::CliEvent::Username(u) => {
                        u.username(crate::state::SSH_USER).unwrap();
                        defmt::debug!("SSH CLIENT: USERNAME SENT");
                    }
                    // ─────────────────────────────────────────────────────────────────
                    // STEP 3: SEND PUBLIC KEY
                    sunset::event::CliEvent::Pubkey(pk) => {
                        let client_key = get_client_key().clone();
                        // BUILD AND LOG THE OPENSSH AUTHORIZED_KEYS
                        let pubkey = client_key.pubkey();
                        if let sunset::PubKey::Ed25519(k) = pubkey {
                            let raw = k.key.0;
                            // SSH WIRE FORMAT: STRING "ssh-ed25519" + STRING RAW KEY
                            let mut blob = alloc::vec::Vec::new();
                            blob.extend_from_slice(&(11u32.to_be_bytes()));
                            blob.extend_from_slice(b"ssh-ed25519");
                            ...
                            blob.extend_from_slice(&raw);
                            let b64 = simple_base64_encode(&blob);
                            defmt::debug!("SSH CLIENT: OFFERING PUBLIC KEY: ssh-ed25519 {}", b64.as_str());
                        }
                        pk.pubkey(client_key).unwrap();
                        defmt::debug!("SSH CLIENT: OFFERED PUBLIC KEY");
                    }
                    // ─────────────────────────────────────────────────────────────────
                    // STEP 4: SIGN KEYS
                    sunset::event::CliEvent::AgentSign(sign) => {
                        let key = sign.key().unwrap();
                        let msg = sign.message().unwrap();

                        let mut buf = [0u8; 1024];
                        let n = sunset::sshwire::write_ssh(&mut buf, &msg)
                            .expect("FAILED TO ENCODE SIGN MESSAGE");
                        let encoded = &buf[..n];

                        // SIGN WITH ED25519 KEY AND WRAP THE RESULT LIKE SUNSET EXPECTS
                        let sig = match key {
                            sunset::SignKey::Ed25519(sk) => {
                                // SIGN USING THE SIGNER TRAIT
                                let signature = ed25519_dalek::Signer::sign(sk, encoded);
                                sunset::OwnedSig::Ed25519(signature.to_bytes())
                            }
                            _ => {
                                defmt::error!("SSH CLIENT: UNSUPPORTED KEY TYPE FOR AGENT SIGN");
                                sign.skip().unwrap();
                                return;
                            }
                        };

                        sign.signed(&sig).unwrap();
                    }
                    // ─────────────────────────────────────────────────────────────────
                    // NOW AUTHENTICATED AND LOGGED INTO THE HOST
                    sunset::event::CliEvent::Authenticated => {
                        defmt::info!("SSH CLIENT: AUTHENTICATED!");
                        break;
                    }
                    // ─────────────────────────────────────────────────────────────────
                    // STEP 5: RECEIVE BANNER
                    sunset::event::CliEvent::Banner(b) => {
                        if let Ok(msg) = b.banner() {
                            defmt::info!("SERVER BANNER: {}", msg);
                        }
                    }
                    sunset::event::CliEvent::PollAgain => {}
                    other => {
                        defmt::info!("SSH CLIENT EVENT: {}", defmt::Debug2Format(&other));
                    }
                },
                Err(e) => {
                    defmt::error!("SSH CLIENT ERROR: {}", defmt::Debug2Format(&e));
                    return;
                }
            }
            drop(ph);
        ...
        // OPEN A PTY SESSION
        let mut channel = match client.open_session_pty().await {
            Ok(ch) => {
                defmt::info!("SSH CLIENT: SHELL SESSION OPENED");
                ch
            }
            Err(e) => {
                defmt::error!("FAILED TO OPEN SESSION: {}", defmt::Debug2Format(&e));
                return;
            }
        };

        // SET TERMINAL SIZE
        let winch = sunset::packets::WinChange {
            cols: 80,
            rows: 24,
            width: 0,
            height: 0,
        };
        if let Err(e) = channel.term_window_change(winch).await {
            defmt::error!("FAILED TO SET WINDOW SIZE: {}", defmt::Debug2Format(&e));
            return;
        }


        let spawner = unsafe { embassy_executor::Spawner::for_current_executor() }.await;
        crate::spawn!(spawner, shell_io_task(channel));

        // CONTINUE POLLING PROGRESS UNTIL SESSION ENDS
        loop {
            match client.progress(&mut ph).await {
                Ok(sunset::event::CliEvent::SessionExit(exit)) => {
                    defmt::info!("REMOTE PROCESS EXITED: {}", defmt::Debug2Format(&exit));
                    break;
                }
                Ok(sunset::event::CliEvent::Defunct) => {
                    defmt::info!("SSH CONNECTION CLOSED");
                    break;
                }
                Ok(_) => {}
                Err(e) => {
                    defmt::error!("SSH CLIENT ERROR AFTER SESSION: {}", defmt::Debug2Format(&e));
                    break;
                }
            }
            drop(ph);
    ...
    embassy_futures::select::select(run_fut, progress_fut).await;
    defmt::info!("SSH CLIENT SESSION ENDED");
}

// I/O TASK
#[embassy_executor::task]
async fn shell_io_task(mut channel: sunset_async::ChanInOut<'static>) {
    embassy_time::Timer::after_millis(500).await;

    let cmd = b"echo 'Hello from SSH!'\n";
    if let Err(e) = embedded_io_async::Write::write(&mut channel, cmd).await {
        defmt::error!("WRITE ERROR: {}", defmt::Debug2Format(&e));
        return;
    }
    if let Err(e) = embedded_io_async::Write::flush(&mut channel).await {
        defmt::error!("FLUSH ERROR: {}", defmt::Debug2Format(&e));
        return;
    }

    // READ AND PRINT ALL OUTPUT UNTIL EOF
    let mut buf = [0u8; 256];
    loop {
        match embedded_io_async::Read::read(&mut channel, &mut buf).await {
            Ok(0) => {
                defmt::info!("REMOTE OUTPUT CLOSED (EOF)");
                break;
            }
            Ok(n) => {
                if let Ok(s) = core::str::from_utf8(&buf[..n]) {
                    defmt::info!("SSH OUTPUT: {}", s);
                } else {
                    defmt::warn!("RECEIVED NON-UTF8 BYTES");
                }
            }
            Err(e) => {
    ...
    if let Err(e) = embedded_io_async::Write::write(&mut channel, b"exit\n").await {
        defmt::error!("EXIT WRITE ERROR: {}", defmt::Debug2Format(&e));
    }
    let _ = embedded_io_async::Write::flush(&mut channel).await;
}
