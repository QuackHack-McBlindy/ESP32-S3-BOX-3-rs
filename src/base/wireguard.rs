    ...
    loop {
        ...
        match cmd {
            WgCommand::Enable => {
                ...
                let private_key_b64 = simple_base64_encode(&config.private_key);
                let peer_pubkey_b64  = simple_base64_encode(&config.peer_public_key);
                defmt::info!("Private key (base64): {}", private_key_b64.as_str());
                defmt::info!("Peer public key (base64): {}", peer_pubkey_b64.as_str());
...
pub struct WgServerConfig {
    pub private_key: [u8; 32],
    pub peers: &'static [ServerPeerConfig],
    pub default_peer: Option<PeerId>,
}


#[embassy_executor::task]
pub async fn wireguard_server(
    wifi_stack: &'static embassy_net::Stack<'static>,
    config: &'static WgServerConfig,
    mut rng: esp_hal::rng::Rng,
    mut runner: ch::Runner<'static, 1420>,
) {
    defmt::info!("🛡️  WireGuard server starting");

    let sk = StaticPrivateKey(config.private_key);
    let mut wg_cfg = Config::new(sk);

    let mut peer_ids: heapless::Vec<PeerId, 8> = heapless::Vec::new();
    for p in config.peers {
        let pk = PublicKey(p.public_key);
        let psk = p.preshared_key.unwrap_or_default();
        let peer_cfg = StaticPeerConfig::new(pk, Some(psk), None);
        let id = wg_cfg.insert_peer(peer_cfg);
        peer_ids.push(id).unwrap();
        defmt::info!("🛡️  Added peer {} with public key {:?}", id, p.public_key);
    }
    ...
    if let Err(e) = socket.bind(embassy_net::IpListenEndpoint {
        addr: None,
        port: 51820, 
    }) {
        defmt::error!("🛡️  Failed to bind UDP socket: {:?}", e);
        return;
    }
    defmt::info!("🛡️  Listening on UDP port 51820");
    
    loop {
        if let Ok(WgCommand::Disable) = WG_CMD.try_receive() {
            defmt::info!("🛡️  Server disabled");
            VPN_ACTIVE.store(false, core::sync::atomic::Ordering::Relaxed);
            runner.set_link_state(LinkState::Down);
            break;
        }
        ...
        if let Some(maintenance) = sessions.turn(now, &mut hw_rng) {
            let data = maintenance.data();
            match socket.send_to(data, socket_addr_to_endpoint(maintenance.to())).await {
                Ok(n) => defmt::debug!("🛡️  Sent {} byte maintenance to {}", n, maintenance.to()),
                Err(e) => defmt::error!("🛡️  Maintenance send failed: {:?}", e),
            }
        }
        ...
        match socket.recv_from(&mut rx_buf.0).await {
            Ok((n, meta)) => {
                ...
                defmt::trace!("🛡️  Received {} bytes from {}", n, src);

                match sessions.recv_message(src, &mut rx_buf.0[..n]) {
                    Ok(Message::Write(response)) => {
                        // HANDSHAKE RESPONSE
                        match socket.send_to(response, socket_addr_to_endpoint(src)).await {
                            Ok(n) => defmt::debug!("🛡️  Sent {} byte handshake response", n),
                            Err(e) => defmt::error!("🛡️  Handshake response send failed: {:?}", e),
                        }
                    }
                    Ok(Message::Read(peer_idx, ip_packet)) => {
                        // DECRYPTED PAYLOAD > FORWARD TO VPN STACK
                        if let Some(mut slot) = runner.try_rx_buf() {
                            let len = ip_packet.len();
                            if len <= slot.len() {
                                slot[..len].copy_from_slice(ip_packet);
                                runner.rx_done(len);
                                defmt::debug!("🛡️  Forwarded {} byte IP packet to TUN", len);
                            } else {
                                defmt::warn!("🛡️  IP packet too big for TUN: {} > {}", len, slot.len());
                            }
                        } else { defmt::warn!("🛡️  RX channel full, dropping packet"); }
                    }
                    Ok(Message::HandshakeComplete(_)) => {
                    }
                    Err(e) => defmt::warn!("🛡️  recv_message error: {:?}", e),
                    _ => {}
                }
            }
            Err(e) => {
                defmt::trace!("🛡️  recv_from error: {:?}", e);
            }
        }

        // OUTGOING PACKETS FROM VPN STACK > ENCRYPT & SEND TO THE APPROPRIATE PEER
        while let Some(slot) = runner.try_tx_buf() {
            let target = match config.default_peer {
                Some(pid) => pid,
                None => {
                    defmt::warn!("🛡️  Outgoing packet but no default_peer configured – dropping");
                    runner.tx_done();
                    continue;
                }
            };

            // ENCRYPT THE WHOLE SLOT  THE SLOT LENGTH IS THE IP PACKET LENGTH
            let len = slot.len();
            let payload = &mut slot[..len]; // mutable borrow
            match sessions.send_message(target, payload) {
                Ok(SendMessage::Data(ep, meta)) => {
                    // PREPARE WIRE FORMAT: HEADER + ENCRYPTED PAYLOAD + LOG
                    meta.frame_in_place(slot);
                    let total_len = core::mem::size_of::<DataHeader>() + len + core::mem::size_of::<Tag>();
                    match socket.send_to(&slot[..total_len], socket_addr_to_endpoint(ep)).await {
                        Ok(n) => defmt::debug!("🛡️  Sent {} byte data packet to {}", n, ep),
                        Err(e) => defmt::error!("🛡️  Data send failed: {:?}", e),
                    }
                }
                Ok(SendMessage::Maintenance(m)) => {
                    // HANDSHAKE REQUIRED – THIS IS UNUSUAL ON THE SERVER SIDE FOR OUTGOING TRAFFIC
                    match socket.send_to(m.data(), socket_addr_to_endpoint(m.to())).await {
                        Ok(n) => defmt::debug!("🛡️  Sent {} byte init handshake (due to send_message) to {}", n, m.to()),
                        Err(e) => defmt::error!("🛡️  Init handshake send failed: {:?}", e),
                    }
                }
                Err(e) => {
                    defmt::error!("🛡️  send_message failed: {:?}", e);
                }
            }
            runner.tx_done();
        }

        // SIGNAL VPN STACK UP IF AT LEAST ONE TRANSPORT IS ACTIVE
        VPN_ACTIVE.store(
            sessions.dynamic.borrow().peers_by_session.values().any(|s| matches!(s.state, SessionState::Transport(_))),
